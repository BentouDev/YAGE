...
namespace Utils
{
    ...
    public:
            ...
            : _memory(memory), _indices(_memory), maxSize(size)
        {
            ...
            elements = reinterpret_cast<object_t*>(memory.allocate(sizeof(object_t) * maxSize, alignof(object_t), DEBUG_SOURCE_INFO));
            memset(reinterpret_cast<void*>(elements), 0, sizeof(object_t) * maxSize);
