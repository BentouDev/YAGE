...
namespace Memory
{
    ...
    void* FreeListAllocator::allocate(std::size_t size, std::size_t alignment, std::size_t offset)
    {
        ...
        YAGE_ASSERT (
            ...
            "FreeListAllocator : Attempt to allocate '{}' bytes in block of size '{}'! Original alloc size '{}'.",
        ...
        if (blockPtr->next != nullptr)
        {
            YAGE_ASSERT (
                ...
                "FreeListAllocator : Memory blocks are damaged, block '{}' exists inside other from '{}' to '{}'.",
                (void*)pNew, (void*)blockPtr, (void*)(blockPtr + blockPtr->size)
            );
        }

        YAGE_ASSERT (
            blockPtr->next != pNew,
            "FreeListAllocator : Deallocation has failed to join adjacent blocks for address '{}'!",
            (void*)pNew
        );
    ...
    void FreeListAllocator::deallocate(void *ptr)
    {
        ...
        YAGE_ASSERT (
            header->adjustment != 0,
            "FreeListAllocator : Cannot deallocate address '{}', its already free!",
            (void*)header
        );
        ...
        *reinterpret_cast<FreeListHeader*>(rawBegin) = freeHeader;
        ...
        YAGE_ASSERT (
            ...
            "FreeListAllocator : Deallocation failed to join adjacent blocks, '{}' is inside other block from '{}' to '{}'.",
            (void*)header->next, (void*)header, (void*)(header + header->size)
        );
